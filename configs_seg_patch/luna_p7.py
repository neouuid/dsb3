...
if zmuv_mean is None or zmuv_std is None:
    ...
    x_big = None
    for i, (x, _, _) in zip(range(4), train_data_iterator.generate()):
        print(i)
...
def conv_prelu_layer(l_in, n_filters, filter_size=3, pad='valid'):
...
def build_model():
    ...
    net['contr_1_4'] = conv_prelu_layer(net['contr_1_3'], base_n_filters)
    net['pool1'] = max_pool3d(net['contr_1_4'])

    net['encode_0_1'] = conv_prelu_layer(net['pool1'], base_n_filters)
    net['encode_0_2'] = conv_prelu_layer(net['encode_0_1'], base_n_filters)
    net['encode_0_3'] = conv_prelu_layer(net['encode_0_2'], base_n_filters)
    net['encode_0_4'] = conv_prelu_layer(net['encode_0_3'], base_n_filters)

    net['encode_1_1'] = conv_prelu_layer(net['pool1'], base_n_filters, filter_size=1)
    net['encode_1_2'] = conv_prelu_layer(net['encode_1_1'], base_n_filters, filter_size=3)

    net['encode_2_1'] = conv_prelu_layer(net['pool1'], base_n_filters, filter_size=1)

    net['encode'] = nn.layers.ConcatLayer([net['encode_0_4'], net['encode_1_2'], net['encode_2_1']],
    ...
    net['dropout_1'] = nn.layers.dropout_channels(net['concat1'], p=0.3)

    net['expand_1_1'] = conv_prelu_layer(net['dropout_1'], base_n_filters)
    ...
    l_out = dnn.Conv3DDNNLayer(net['expand_1_4'], num_filters=1,
                               filter_size=1,
