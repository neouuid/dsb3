...
def nd_window(data, filter_function):
    """
...
def alternative_data_view(data):
    shape = data.shape
    assert(len(shape)==4)
    data_cropped = data[:,:shape[1]-1,:shape[2]-1,:shape[3]-1]
    weights = nd_window(data_cropped, scipy.signal.hamming)
    x = weights * data_cropped
    x = np.fft.fftn(x, s=None, axes=(-3, -2, -1))
    x = np.absolute(x)
    #crop out one of the symmetric cubes
    x = x[:,:x.shape[1]//2+1,:x.shape[2]//2+1,:x.shape[3]//2+1]
    x = np.float32(x)
    return x
...
conv3d_valid = partial(dnn.Conv3DDNNLayer,
                 filter_size=3,
                 pad='valid',
                 W=nn.init.HeNormal('relu'),
                 b=nn.init.Constant(0.01),
...
def build_model():
    ...
    l_alt_in = nn.layers.InputLayer((None, ) + p_transform['2nd_patch_size'])
    l_alt_in_ds = nn.layers.DimshuffleLayer(l_alt_in, pattern=[0,'x',1,2,3])
    ...
    l1 = dense(drop(l1), 512)

    l2 = conv3d_valid(l_alt_in_ds, 64)
    l2 = conv3d_valid(l2, 64, stride=2)
    
    l2 = conv3d_valid(l2, 64)
    l2 = conv3d_valid(l2, 64)
    l2 = conv3d_valid(l2, 64, stride=2)

    l2 = dense(drop(l2), 128)

    l = lasagne.layers.ConcatLayer([l1, l2], axis=1, name='concat_layer_b1_b2')
    ...
    return namedtuple('Model', ['l_in', 'l_alt_in', 'l_out', 'l_target', 'l_enable_target'])(l_in, l_alt_in, l_out, l_target, l_enable_target)
