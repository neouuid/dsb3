...
def build_model():
    ...
    l_reshape = nn.layers.ReshapeLayer(l, (-1, n_candidates_per_patient, 1), name='reshape2patients')

    l_agg_mil_loss = nn_lung.AggMILLoss(l_reshape, name='aggregate_mil_loss')

    l_out = nn_lung.AggAllBenignExp(l_reshape, name='aggregate_all_nodules_benign')

    return namedtuple('Model', ['l_in', 'l_out', 'l_agg_mil_loss', 'l_target'])(l_in, l_out, l_target)
